...
if __name__ == "__main__":
    ...
    org_fed_cost = [None for j in range(len(OldEnvironment.Environment.providers))]
    for j in range(len(OldEnvironment.Environment.providers)):
        org_fed_cost[j] = {}
        for k in OldEnvironment.Environment.domain.services:
            org_fed_cost[j][k] = OldEnvironment.Environment.providers[j].federation_costs[k]

    while i <= scale:
        for j in range(len(OldEnvironment.Environment.providers)):
            for k in OldEnvironment.Environment.domain.services:
                OldEnvironment.Environment.providers[j].federation_costs[k] = org_fed_cost[j][k] * (init_mult + i * step)
        ...
        for j in range(iterations):
            ...
            demands = OldEnvironment.Environment.generate_req_set(sim_time * 5)
