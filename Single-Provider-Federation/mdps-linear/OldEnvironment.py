...
class Env:
    ...
    def step(self, state, action):
        ...
        #generate the next state
            ...
            #heapq.heapify(self.events)
            if Environment.verbose:
                Environment.debug("processing event in dep_continue: time = ", event.time, ", type = ", event.event_type ,", req = ", event.req)
...
def print_events(events):
    ...
    for i in range(len(events)):
        e = events[i]
        Environment.debug("time = ", e.time, ", type = ", e.event_type ,", req = ", e.req)
