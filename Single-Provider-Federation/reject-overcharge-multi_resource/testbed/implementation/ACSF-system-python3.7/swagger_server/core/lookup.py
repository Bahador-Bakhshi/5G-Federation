...
def get_service_aggregated_requirements(request):
    vnfs = request.vn_fs #swagger codegen convert "vnfs" to "vn_fs" !!!
    cpu = 0
    ram = 0
...
def random_valid_action(request, active_services, available_resources):
    valid_domains = get_valid_domains(request, available_resources)
