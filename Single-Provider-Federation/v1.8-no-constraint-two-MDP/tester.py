...
import parser
#import DP
from Environment import debug, error, warning, verbose
...
def test_policy(demands, policy):
    ...
    for i in range(len(demands)):
        ...
        if last_state == ((0,1),(0,1)):
            if not(state in next_visited):
                next_visited.update({state: 1})
            else:
                c = next_visited[state]
                c += 1
                next_visited.update({state: c})
        last_state = state
