...
def create_SAC_agent(collect_env):
...
def create_random_policy(collect_env):
...
def create_learner(tf_agent, experience_dataset_fn):
...
def create_eval_actor(eval_env, eval_policy, train_step):
...
def create_replay_buffer(tf_agent):
    table = reverb.Table(
        table_name,
...
def create_collect_driver(train_env, collect_policy, rb_observer):
...
def create_rb_observer(reverb_replay):
...
def create_init_collect_driver(rb_observer, collect_env, random_policy):
...
def create_iterator(reverb_replay):
...
def train(agent, train_env, eval_env):
    ...
    collect_policy, eval_policy = create_policies(agent)
    eval_actor = create_eval_actor(eval_env, eval_policy, train_step)
    metrics = get_eval_metrics(eval_actor)

    log_eval_metrics(0, metrics)

    replay_buffer = create_replay_buffer(agent)
    if debug > 1:
        print("replay_buffer is created ")
   
    rb_observer = create_rb_observer(replay_buffer)
    if debug > 1:
        print("rb_observer = ", rb_observer)

    random_policy = create_random_policy(train_env)
    init_driver = create_init_collect_driver(rb_observer, train_env, random_policy)
    if debug > 1:
    ...
    collect_actor = create_collect_driver(train_env, collect_policy, rb_observer)
    if debug > 1:
    ...
    agent_learner = create_learner(agent, iterator)
