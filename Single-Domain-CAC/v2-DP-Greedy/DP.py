...
class Request:
    ...
    def __str__(self):
        return "w = "+ str(self.w) +" st = "+ str(self.st) +" dt = "+ str(self.dt) +" rev = "+ str(self.rev) +", index = "+ str(self.index)
...
def test_dp_policy(demands, policy):
    profit = 0
    accepted = []
    capacity = server_size
    for i in range(len(demands)):
...
if __name__ == "__main__":
    demands = generate_req_set(total_classes, 1000)
    print_reqs(demands)
    greedy_profit = test_greedy_policy(demands)
    print("Greedy Profit = ", greedy_profit)

    policy = DP()
    dp_profit = test_dp_policy(demands, policy)
    print("DP Profit = ", dp_profit)

    print("Profit Ratio = ", float(greedy_profit) / float(dp_profit))
