...
def create_envs(req_num = 0, requests = None):
    ...
    train_py_env = tf_env_wrapper.TF_Agent_Env_Wrapper(req_num = req_num, requests = requests)
    eval_py_env  = tf_env_wrapper.TF_Agent_Env_Wrapper(req_num = req_num, requests = requests)
...
def compute_avg_return(environment, policy, num_episodes=0):
    total_return = 0.0
...
def main():
    train_env, eval_env = create_envs(req_num = req_per_episode)
